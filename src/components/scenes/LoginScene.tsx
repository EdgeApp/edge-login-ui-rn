...
export const LoginScene = (props: Props): React.ReactElement => {
  ...
  const initialPinEnabled = hasPinOrBiometric(initialUser)
  ...
  const activeUser = React.useMemo<LoginUserInfo | undefined>(
    () => findUser(localUsers, activeLoginId, activeUsername),
  ...
  const handleSelectUser = useHandler((userInfo: LoginUserInfo) => {
    ...
    const nextPinEnabled = hasPinOrBiometric(userInfo)
    ...
    Airship.show(bridge => (
      ...
      .then(async button => {
        ...
        if (userInfo.loginId === activeLoginId) {
          ...
          setPasswordErrorMessage(undefined)
          setUsernameErrorMessage(undefined)
          const nextPinEnabled = hasPinOrBiometric(nextUser)
  ...
  const handleTroubleSigningIn = useHandler(() => {
    // Recovery and QR login both complete a login of their own, so starting one
    // while a password/PIN attempt is still settling would race two logins:
    if (isLoginInFlight) return
    Keyboard.dismiss()
  ...
  const handleKeypadPress = useHandler((value: string) => {
    // `PinKeypad` only forwards `disabled` to its backspace key, so the digits
    // stay pressable during a lockout countdown or an in-flight attempt. Guard
    // here as well, or each re-entry would fire another login at the server:
    if (hasWait || isLoginInFlight) return
    const newPin =
  ...
  React.useEffect(() => {
    ...
    if (!hasPickedFlavorRef.current) {
      const seedPinEnabled = hasPinOrBiometric(seedUser)
...
/** PIN and biometric are one login flavor, so either one enables the pill. */
const hasPinOrBiometric = (user: LoginUserInfo | undefined): boolean =>
  user != null && (user.pinLoginEnabled || user.touchLoginEnabled)

/** Resolve a saved account by loginId, falling back to an exact username. */
const findUser = (
...
const findSeedUser = (
  ...
  const user = findUser(localUsers, loginId, username)
  if (user != null) return user

  // A username in the route pins the account even when it is not saved locally
  // (an OTP retry re-enters a typed username), so only an empty one may fall
  // back:
  ...
  // whose only saved account is unnamed leaves `activeUser` unset, which greys
  // out the PIN/biometric flavor the account actually supports.
  return localUsers[0]
}
