...
>((props: FilledTextInputProps, ref) => {
  const {
    // Contents:
    error,
    placeholder,
  ...
  const focusValue = useDerivedValue(
    () => (hasValue ? 1 : focusAnimation.value),
    [hasValue]
  )
        ...
        >
          ...
          {secureTextEntry ? (
            ...
            >
              <IconContainer>
                {hidePassword ? (
                  <EyeOffIconAnimated accessible color={iconColor} />
                ) : (
                  <EyeIconAnimated accessible color={iconColor} />
                )}
              </IconContainer>
            </TouchableWithoutFeedback>
          ) : null}
...
const Placeholder = styled(Animated.View)<{ shift: SharedValue<number> }>(
  theme => ({ shift }) => {
    ...
    return [
      ...
      useAnimatedStyle(() => ({
        transform: [
          { translateY: interpolate(shift.value, [0, 1], [0, -1.2 * rem]) }
        ]
      }))
    ]
  }
